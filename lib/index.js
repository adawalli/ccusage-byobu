...
export async function main() {
  try {
    ...
    if (config.format === 'full') {
      ...
      // Calculate total width without color codes
      const baseOutput = `${progressBar} ${timeStr} ${formattedCost}`;

      if (baseOutput.length > maxWidth) {
        // Truncate by removing progress bar first
        output = `${colorCode}${timeStr} ${formattedCost}${resetColor}`;
      } else {
        output = `${colorCode}${progressBar} ${timeStr} ${formattedCost}${resetColor}`;
      }
    } else {
