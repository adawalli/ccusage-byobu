...
export async function main() {
  try {
    ...
    function getColorCode(usagePercent) {
      const colors = process.env.CCUSAGE_BYOBU_COLORS || 'auto';
    ...
    const colorCode = getColorCode(usagePercent);
    ...
    // Format output for byobu (max 25 chars, include color codes)
    const output = `${colorCode}${progressBar} ${timeStr} $${currentCost.toFixed(2)}${resetColor}`;
