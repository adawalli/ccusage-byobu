...
export async function main() {
  try {
    ...
    // Check if cache should be disabled (for byobu status scripts)
    const cacheDisabled =
      process.env.CCUSAGE_DISABLE_CACHE === '1' || process.env.CCUSAGE_DISABLE_CACHE === 'true';
    ...
    if (!jsonOutput) {
      ...
      try {
        ...
        if (process.env.CCUSAGE_BYOBU_DEBUG) {
          const cacheStatus = cacheDisabled ? '(cache disabled)' : '';
          console.error(
