...
class Cache extends EventEmitter {
  constructor(options = {}) {
    ...
    if (process.env.CCUSAGE_BYOBU_CACHE_CLEANUP_INTERVAL) {
      const parsed = parseInt(process.env.CCUSAGE_BYOBU_CACHE_CLEANUP_INTERVAL, 10);
      if (!isNaN(parsed) && parsed > 0) {
        envConfig.cleanupIntervalMs = parsed;
      }
    }

    if (process.env.CCUSAGE_BYOBU_CACHE_MAX_KEYS) {
      const parsed = parseInt(process.env.CCUSAGE_BYOBU_CACHE_MAX_KEYS, 10);
      if (!isNaN(parsed) && parsed > 0) {
        envConfig.maxKeys = parsed;
      }
    }

    if (process.env.CCUSAGE_BYOBU_CACHE_WINDOW_SIZE) {
      const parsed = parseInt(process.env.CCUSAGE_BYOBU_CACHE_WINDOW_SIZE, 10);
      if (!isNaN(parsed) && parsed > 0) {
        envConfig.windowSize = parsed;
      }
    }
    ...
    if (!Number.isInteger(this.config.cleanupIntervalMs) || this.config.cleanupIntervalMs <= 0) {
      throw new Error(
        `cleanupIntervalMs must be a positive integer, got: ${this.config.cleanupIntervalMs}`
      );
    }
    if (
      this.config.maxKeys !== null &&
      (!Number.isInteger(this.config.maxKeys) || this.config.maxKeys <= 0)
    ) {
      throw new Error(`maxKeys must be a positive integer or null, got: ${this.config.maxKeys}`);
    }
    if (!Number.isInteger(this.config.windowSize) || this.config.windowSize <= 0) {
      throw new Error(`windowSize must be a positive integer, got: ${this.config.windowSize}`);
    }
    ...
    // Cleanup timer to prevent memory leaks (only if caching is enabled)
    const cacheEnabled =
  ...
  set(key, value, ttlMs = 15000) {
    ...
    if (this.store.has(key)) {
      ...
      if (oldestKey !== undefined) {
        ...
        // Handle case where Maps are out of sync
        console.warn('Cache Maps out of sync: no oldest key found despite size check');
      }
    }
  ...
  _getValueSize(value) {
    try {
      // Use a circular reference-safe serializer
      ...
      // Calculate actual byte size using Buffer.byteLength for accurate UTF-8 encoding
  ...
  getMemoryUsage() {
    ...
    for (const [key, entry] of this.store.entries()) {
      // Use consistent size calculation methods
      totalSize += Buffer.byteLength(key, 'utf8'); // Accurate key size
      totalSize += this._getValueSize(entry.value); // Use existing method for value size
 ...
 * Clears the existing cache and allows a fresh instance to be created
 */
